...
import { createIcon } from '../basics/Icon';
...
export const createHeader = ({ user, onLogout, onLogin, onCreateAccount }) => {
  ...
  if (user) {
    account.appendChild(createIcon({ size: 'small', label: 'Log out', onClick: onLogout }));
  } else {
    account.appendChild(createIcon({ size: 'small', label: 'Log in', onClick: onLogin }));
    account.appendChild(
      createIcon({
        size: 'small',
        label: 'Sign up',
